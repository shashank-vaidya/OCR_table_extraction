    ...
    def crop_each_bounding_box_and_ocr(self):
        ...
        for row in self.rows:
            for bounding_box in row:
                ...
                results_from_ocr = self.get_result_from_paddleocr(image_slice_path)
                current_row.append(results_from_ocr)
                image_number += 1
            self.table.append(current_row)
            current_row = []

    def get_result_from_paddleocr(self, image_path):
...
# Usage example (You need to adjust the image paths)
# image = cv2.imread('path_to_thresholded_image.jpg')
# original_image = cv2.imread('path_to_original_image.jpg')
# tool = OcrToTableTool(image, original_image)
# tool.execute()
